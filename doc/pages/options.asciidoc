...
*ui_options* `str-to-str-map`::
    ...
    interface implementation. The Terminal UI supports the following options:
        ...
        *terminal_info_max_height*:::
            set the maximum allowable height, in content lines, of an info
            box. set to zero for no limit. content that does not fit stays
            reachable with the `info-scroll-down` and `info-scroll-up`
            commands.
