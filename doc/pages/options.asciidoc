...
*ui_options* `str-to-str-map`::
    ...
    interface implementation. The NCurses UI support the following options:
        ...
        *terminal_set_title*:::
            If set to *no* or *false*, the terminal emulator title won't
            be changed. If set to *yes* or *true*, each window's title will
            mirror the contents of the window's modeline (as set by the 
            `modelinefmt` option), replacing non-ASCII characters with question
            marks. Otherwise, each window will use the value of this option at
            window scope as the title text, without further expansion.
