...
Some keys cannot be mapped directly but they can still mapped by `c-v` to directly insert their keycode as an argument to map.
...
In order to map these keys type `<c-v>` before the key in the command. For example:
