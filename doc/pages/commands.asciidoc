...
*enter-user-mode* [<switches>] <name>::
    ...
    *-count <count>*:::
        Forward *count* to the underlying normal mode parameters

    *-register <register>*:::
        Forward *register* to the underlying normal mode parameters
...
*info* [<switches>] <text>::
    ...
    *-keep*:::
        keep the info box that is currently displayed instead of
        displaying a new one. Info boxes are automatically hidden on
        the next keystroke, this cancels that, so that a command bound
        to a key can act on the info box that is displayed rather than
        on the one it just dismissed. All other switches and the text
...
*info-scroll* [<count>]::
    scroll the info box of the current client by <count> lines, so that
    content that does not fit on screen can be reached. A negative
    <count> scrolls up, a positive one scrolls down; <count> is 1 by
    default. A dotted border edge means there is more content in that
    direction, and the right border doubles as a scrollbar showing which
    part of the content is on screen, like the one on the completion
    menu. The scroll position is kept by the client, and is reset
    whenever a different info box is displayed. If the info box got
    hidden (for instance by opening the command prompt to run this very
    command) it is displayed again. +
    This is not a builtin command but one defined in the standard
    `kakrc`, so it is not available when running with `kak -n`.
