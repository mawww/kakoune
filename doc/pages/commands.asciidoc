...
*info-scroll-down* [<count>]::
    scroll the info box of the current client down by <count> lines,
    1 by default, so that content that does not fit on screen can be
    reached. A dotted border edge means there is more content in that
    direction. The scroll position is kept by the client, and is reset
...
*info-scroll-up* [<count>]::
    scroll the info box of the current client up by <count> lines,
    1 by default. See *info-scroll-down*.
