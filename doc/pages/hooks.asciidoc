...
*ClientCreate* `client name`::
    Triggered when a new client is created, just before WinDisplay is called.

*ClientClose* `client name`::
    Triggered just before a client is closed. At this point the client is still in
    a valid state, and is present in the `$kak_client_list` variable.
