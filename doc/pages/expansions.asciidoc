...
*%val{scratch}*::
    _in buffer, window scope_ +
    `true` if the buffer is not linked to a file ("scratch"), otherwise `false`

*%val{new_file}*::
    _in buffer, window scope_ +
    `true` if the buffer is linked to a file that does not exist yet, otherwise `false`

*%val{fifo}*::
    _in buffer, window scope_ +
    `true` if the buffer is currently linked and reading from a fifo, otherwise `false`
