...
Expansions with the type `env` will expand to the content of the environment
variable given in the argument.

For example, `%env{HOME}` will expand to the path of the user's home, which is
equivalent to `%sh{echo "$HOME"}`.
