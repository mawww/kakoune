...
* draw_status(Line prompt, Line content, int cursor_pos, Line mode_line, Face default_face, String context)
  ...
  context indicates what kind of status is being displayed:
  - "command": command prompt (:)
  - "search": search prompt (/, ?)
  - "prompt": general prompt (e.g. pipe, object desc)
  - "": normal status (file info, echo messages)
