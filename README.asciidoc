...
Kakoune's builtin `colorscheme` command will provide completion for
color schemes installed to the directories defined in the
`colorscheme_sources` option. By default, it will source colorschemes
from `../share/kak/colors/`, then `$XDG_CONFIG_HOME/kak/colors/`
(if it exists). Schemes duplicated in later directories in this list
