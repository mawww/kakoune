...
commands are similar to those of vi and it shares Vi's "keystrokes as
...
Kakoune works on selections, which are a direction-oriented, inclusive range of characters
which have an anchor and a cursor character. Most commands move both the anchor
and the cursor, except when extending the selection where the anchor character stays fixed
and the cursor is moved.
