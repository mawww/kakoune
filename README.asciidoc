...
to recursively source any `.kak` files matching
`$XDG_CONFIG_HOME/kak/autoload/*.kak` (with `$XDG_CONFIG_HOME` defaulting to
`$HOME/.config`, and falling back to `../share/kak/autoload/` if that autoload
directory does not exists), then `../share/kak/kakrc.local` if it exists, and
