...
with `scope` being one of `global`, `buffer` or `window`, mode being `insert`,
`normal`, `prompt`, `menu`, `goto`, `view`, `object`, `user` or a one character
long arbitrary prefix, `key` being a single key name and `keys` a list of keys.

`user` mode allows for user mappings behind the `,` key.

All mappings, except those bound to the `insert` mode, will be executed in
normal mode.
