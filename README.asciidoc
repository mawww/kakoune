...
In each client, every change to the selections' extent can be undone.
This allows to recover from mistakes. Additionally, some commands like the goto
commands, buffer switch or search commands mark the previous selections as jump
target. It is possible to undo selection changes until the next jump target.

See <<doc/pages/keys#selection-undo,`:doc keys selection-undo`>>.

Marks
~~~~~

The extent of the current selections can also be saved in a register and restored later on.
