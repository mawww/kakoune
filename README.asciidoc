...
Kak was designed from the start to handle multiple selections (also called multiple cursors).
 ...
 * `s`: make a cursor for each match of the pattern in the current selections
 * `S`: split selection into new cursors for each match of the pattern
 * `alt-s`: split selection into new cursors
 * `C`: make new cursors below the current ones
 * `alt-C`: make new cursor above the current ones
 * `$`: filter selections by a pipe: keeps the ones returning 0
 * `alt-k`: filter selections by regex (keeps the matches)
 * `alt-K`: filter out selections by regex (removes the matches)
 * `space`: remove all cursors but the main one (with count n: keep the nth one)
 * `alt-space`: remove one cursor

A recap: another way to get more cursors is by adding the current search pattern:
 ...
 * `alt-i`: selects the inner object, that is the object excluding its surrounder.
 ...
 * `"`: Prefix to set the current register
 * `ctrl-r`: (in insert mode) Paste the contents of register
 ...
 * `alt-*`: same as above, but without trying to be smart
...
The `user` mode allows for user mapping behind the `,` key. Keys will be executed in
normal mode. So use `exec` to run default key bindings, example:

    # map ,/ to search in selections (s), even if s is rebound in normal mode:
    map global user / ":exec s<ret>"
 ...
 * `-shell-params`:      pass parameters to each shell escape as $0..$N
 * `-allow-override`:    allow overriding an existing command
 * `-hidden`:            do not display the command in completion candidates
 * `-docstring`:         define the documentation string for command
 ...
 * `-shell-completion`:  complete the parameters using the given shell-script
