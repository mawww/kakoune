...
std::chrono::milliseconds get_next_key_idle_timeout(const Context& context)
{
    return std::chrono::milliseconds{context.options()["next_key_idle_timeout"].get<int>()};
}
...
public:
    NextKey(InputHandler& input_handler, String name, KeymapMode keymap_mode, KeyCallback callback,
        ...
        : InputMode(input_handler), m_name{std::move(name)}, m_callback(std::move(callback)), m_keymap_mode(keymap_mode),
          m_idle_timer{Clock::now() + get_next_key_idle_timeout(context()), std::move(idle_callback)} {}
