...
add-highlighter -group / regions -default code gas \
    string         '"' (?<!\\)(\\\\)*"        '' \
    commentMulti   /\*       \*/              '' \
    commentSingle1 '#'       '$'              '' \
    commentSingle2 ';'       '$'              ''

add-highlighter -group /gas/string fill string
add-highlighter -group /gas/commentMulti fill comment
add-highlighter -group /gas/commentSingle1 fill comment
add-highlighter -group /gas/commentSingle2 fill comment

# Constant
add-highlighter -group /gas/code regex (0[xX][0-9]+|\b[0-9]+)\b 0:value

# Labels
add-highlighter -group /gas/code regex ^\h*([A-Za-z0-9_.-]+): 0:operator

# ARM Directives
add-highlighter -group /gas/code regex ((^|\s+)\.([248]byte|align|arch(_extension)?|arm|bsscantunwind|code|[cf]pu|[dq]n|eabi_attribute|even|extend|ldouble|fnend|fnstart|force_thumb|handlerdata|inst(\.[nw])?|ltorg|movsp|object_arch|packed|pad|personality(index)?|pool|req|save|setfp|screl32|syntax|thumb(_func|_set)?|tlsdescseq|unreq|unwind_raw|vsave)(\h+|$)) 0:type
...
# Registers
add-highlighter -group /gas/code regex \%(([re](ax|bx|cx|dx|si|di|bp|sp))|(al|bl|cl|dl|sil|dil|bpl|spl)|(r[8-9][dwb])|(r1[0-5][dwb])|(cs|ds|es|fs|gs|ss|ip|eflags)|([xy]mm[0-9]|[xy]mm1[0-5]))\b 0:identifier

# General Instructions
add-highlighter -group /gas/code regex \
