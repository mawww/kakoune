...
def -hidden c-trim-autoindent %[ eval -draft -itersel %[
    # remove the line if it's empty when leaving the insert mode
    try %[ exec <a-x> 1s^(\h+)$<ret> d ]
] ]

def -hidden c-indent-on-newline %< eval -draft -itersel %<
    exec \;
    try %<
...
def -hidden c-indent-on-opening-curly-brace %[
    # align indent with opening paren when { is entered on a new line after the closing paren
    try %[ exec -draft -itersel h<a-F>)M <a-k> \`\(.*\)\h*\n\h*\{\' <ret> s \`|.\' <ret> 1<a-&> ]
]

def -hidden c-indent-on-closing-curly-brace %[
    # align to opening curly brace when alone on a line
    try %[ exec -itersel -draft <a-h><a-:><a-k>^\h+\}$<ret>hms\`|.\'<ret>1<a-&> ]
]

def -hidden c-insert-on-closing-curly-brace %[
    # add a semicolon after a closing brace if part of a class, union or struct definition
    try %[ exec -itersel -draft hm<a-x>B<a-x><a-k>\`\h*(struct|union)<ret> a\;<esc> ]
]

def -hidden c-insert-on-newline %[ eval -draft %[
    exec \;
    try %[
        eval -draft %[
...
add-highlighter -group / regions -default code -match-capture c \
    string %{(?<!')"} %{(?<!\\)(?:\\\\)*"} "" \
    string %{R"([^(]*)\(} %{\)([^)]*)"} "" \
    comment /\* \*/ "" \
    comment // $ "" \
...
add-highlighter -group /c/string fill string
add-highlighter -group /c/comment fill comment
add-highlighter -group /c/disabled fill rgb:666666
add-highlighter -group /c/macro fill meta
add-highlighter -group /c/macro regex ^\h*#include\h+(\S*) 1:module
...
add-highlighter -group /c/code regex \b(__STDC_HOSTED__|__FILE__|__LINE__|__DATE__|__TIME__)\b 0:builtin

hook global WinSetOption filetype=c %[
    hook -group c-indent window InsertEnd .* c-trim-autoindent
    hook -group c-insert window InsertChar \n c-insert-on-newline
    hook -group c-indent window InsertChar \n c-indent-on-newline
    hook -group c-indent window InsertChar \{ c-indent-on-opening-curly-brace
    hook -group c-indent window InsertChar \} c-indent-on-closing-curly-brace
    hook -group c-insert window InsertChar \} c-insert-on-closing-curly-brace

    alias window alt c-alternative-file
]

hook global WinSetOption filetype=(?!c$).* %[
    remove-hooks window c-hooks
    remove-hooks window c-indent
    remove-hooks window c-insert

    unalias window alt c-alternative-file
]
...
def -hidden c-insert-include-guards %{
    %sh{
...
hook global BufNewFile .*\.h c-insert-include-guards

decl -docstring "colon separated list of path in which header files will be looked for" \
    str-list alt_dirs ".:.."

def c-alternative-file -docstring "Jump to the alternate file (header/implementation)" %{ %sh{
    alt_dirs=$(printf %s\\n "${kak_opt_alt_dirs}" | tr ':' '\n')
    file="${kak_buffile##*/}"
    file_noext="${file%.*}"
    ...
    case ${file} in
        ...
        *.h|*.inl)
            for alt_dir in ${alt_dirs}; do
                altname="${dir}/${alt_dir}/${file_noext}.c"
                if [ -f "${altname}" ]; then
                    printf 'edit %%{%s}\n' "${altname}"
                    exit
                fi
