...
declare-option -hidden bool editorconfig_insert_final_newline false
        ...
        editorconfig "${1:-$kak_buffile}" | awk -F= -- '
            ...
            END {
                ...
                if (insert_final_newline == "true") {
                    print "set-option buffer editorconfig_insert_final_newline true"
                }
            }
        '
    }
    hook buffer BufWritePre %val{buffile} -group editorconfig-hooks %{
        evaluate-commands %sh{
        ...
        evaluate-commands -draft %{
            evaluate-commands %sh{
                if [ ${kak_opt_editorconfig_insert_final_newline} = "true" ]; then
                    printf %s\\n "try %{ execute-keys %{ geh } }"
                fi
            }
            evaluate-commands %sh{
                if [ ${kak_opt_editorconfig_insert_final_newline} = "true" ] \
                    && [ ${kak_cursor_char_value} != '10' ]; then
                    printf %s\\n "try %{ execute-keys %{ gj<a-o> } }"
                fi
            }
        }
    }
